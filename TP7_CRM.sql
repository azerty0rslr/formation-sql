...
# SELECT facture.reference AS ref,
# client.nom AS clients,
# facture.info AS info,
# facture.total AS total,
# facture.date_crea AS dates,
# facture.date_paiement AS paiement
# FROM facture LEFT JOIN devis ON facture.devis_id = devis.id 
# LEFT JOIN projet ON devis.projet_id = projet.id 
# LEFT JOIN client ON projet.client_id = client.id

# 2-  Afficher le nombre de factures par client
# SELECT client.nom AS client,
# COUNT(facture.id) AS nb_factures FROM facture INNER JOIN devis ON facture.devis_id = devis.id 
...
# 4- Afficher le CA total avec SUM() pour additioner les champs
# SELECT SUM(facture.total) AS ca_total FROM facture INNER JOIN devis ON facture.devis_id = devis.id
# INNER JOIN projet ON devis.projet_id = projet.id 
# RIGHT JOIN client ON projet.client_id = client.id

# 5- Afficher la somme des factures en attente de paiement
# SELECT SUM(facture.total) AS total_factures FROM facture INNER JOIN devis ON facture.devis_id = devis.id
# INNER JOIN projet ON devis.projet_id = projet.id 
# RIGHT JOIN client ON projet.client_id = client.id
# WHERE facture.date_paiement IS NULL

# 6- Afficher les factures en retard de paiment
# SELECT facture.reference AS facture,
# DATEDIFF(NOW(), facture.date_crea) AS nb_jour
# FROM facture
# WHERE facture.date_paiement IS NULL;

# 7- Afficher les factures en retard de paiment avec le nom du client
# SELECT facture.reference AS facture,
# client.nom AS client,
# DATEDIFF(NOW(), facture.date_crea) AS nb_jour
# FROM facture INNER JOIN devis ON facture.devis_id = devis.id
# INNER JOIN projet ON devis.projet_id = projet.id 
# INNER JOIN client ON projet.client_id = client.id
# WHERE facture.date_paiement IS NULL

# 8- Ajouter une pénalité de 2 euros par jours de retard
# SELECT facture.reference AS facture,
# DATEDIFF(NOW(), facture.date_crea) AS nb_jour_retard,
# DATEDIFF(NOW(), facture.date_crea) * 2 AS penalite
# FROM facture WHERE facture.date_paiement IS NULL;

# 9- Afficher la moyenne de CA par client 
